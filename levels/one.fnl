...
(fn midboss-one []
	...
	(fn spawn-bullets-1 [enemy]
		(fn spawn-bullets [alt]
			...
			(set enemy.flags.bullet-angle-2 (+ enemy.flags.bullet-angle-2 (/ g.tau count))))
	...
	(fn spawn-bullets-2 [enemy]
		...
		(fn spawn-bullets []
			...
			(set enemy.flags.bullet-mod-1 (- enemy.flags.bullet-mod-1 0.00025)))
		(when (= (% enemy.flags.bullet-clock max) 0)
			(set enemy.flags.bullet-mod-1 (/ g.phi 4))
		...
		(fn spawn-bullets []
			...
			(for [i 1 count]
				(when (>= i (/ count 2))
					(class-bullet.spawn (fn [bullet]
						(set bullet.x enemy.x)
						(set bullet.y boss-offset-bottom)
						(set bullet.type 8)
						(set bullet.top true)
						(set bullet.speed 1)
						(set bullet.angle angle))))
						(set angle (+ angle (/ g.tau count))))
				(set enemy.flags.bullet-angle-2 (- enemy.flags.bullet-angle-2 (/ g.phi 10))))
		(when (= (% enemy.flags.bullet-clock max) start) (set enemy.flags.bullet-angle-2 0))
		(when (and (= (% enemy.flags.bullet-clock s-interval) 0) (>= (% enemy.flags.bullet-clock max) start) (< (% enemy.flags.bullet-clock max) end))
...
(fn wave-three []
	(fn spawn-enemy [x y shooter alt]
		(class-enemy.spawn (fn [enemy]
			...
			(set enemy.x (- (+ (* x 48) g.width) 16))
			(set enemy.y y)
			(set enemy.type (if alt :robo-blue :robo-red))
			(set enemy.angle math.pi)
			(when shooter (set enemy.flags.shooter true))
			(set enemy.speed 1))
			(fn [enemy]
...
(fn update-enemies []
	...
	(when (= clock (+ base (* 60 3))) (wave-three))

	(set base (+ base (* 60 7.75)))
	(when (= clock base) (midboss-one))
...
(var block-layout [
	...
	; [:x :X :x :o :x :X :x :_ :_ :_ :o :x :X :o :_]
	; [:X :x :X :o :X :x :X :< :- :- :x :X :x :X :<]
	; [:x :X :x :o :x :X :x :_ :_ :_ :X :x :X :x :_]
...
; 		(class-enemy.spawn (fn [enemy]
; 			(set enemy.health 2)
; 			(set enemy.flags.c-x (math.floor (* 3 (/ g.width 5))))
; 			(set enemy.flags.c-y (/ g.height 2))
; 			(set enemy.flags.c-angle angle)
; 			(set enemy.flags.distance (* g.grid 4))
